...
class ProxyDownladerMiddleware(object):
    def __init__(self):
        self.redis_server = Redis(decode_responses=True)
        self.ua = UserAgent()

    def process_request(self,request,spider):
        # 1
        # ip = self.redis_server.srandmember("ip_pool",1)
        # print("*"*10)
        # print(ip)
        # ret = self.verify_ip(ip)
        # if ret == 1:
        #     print("验证通过")
        #     ip = ip[0].replace("http","https")
        #     print("当前ip%s" %ip)
    ...
    def verify_ip(self,ip):
        ...
        print(ip)
        proxies = {"http":ip}
        # proxies = {"http": 'http://164.100.146.93:80'}
        try:
